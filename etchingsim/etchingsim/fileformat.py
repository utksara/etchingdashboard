import vtk
from . import fft_reconstruct, mra_reconstruct, point_reconstruct
import numpy as np
...
def get_vtp_points(vtp_file_path: str) -> list[tuple[float, float]]:
    ...
    for i in range(num_points):
        ...
        svg_points.append((point[0], point[1]))
    return svg_points

def points_to_svg(points: list[tuple[float, float]], svg_file_path: str, stroke_width : float = 0.01) -> None:
    """
    ...
    extracted_points = list(zip(*points))
    x_coords = extracted_points[0]
    y_coords = extracted_points[1]
    ...
    stroke_width = len(points)*0.0001
    ...
    svg_content += f""" <path d = \" M {points[0][0]},{points[0][1]}"""
    for i in range(1, len(points)):
        point = points[i]
        # p0 = points[i - 1]
        # p1 = points[i]

        # cp1x = p0[0] + (p1[0] - p0[0]) / 3
        # cp1y = p0[1]

        # cp2x = p1[0] - (p1[0] - p0[0]) / 3
        # cp2y = p1[1]
        
        # svg_content += f""" C {cp1x},{cp1y} {cp2x},{cp2y} {p1[0]},{p1[1]}"""
        # """
        svg_content += f""" L {point[0]},{point[1]} """
        """
    ...
    with open(svg_file_path, "w") as f:
        f.write(svg_content)
    # with open("dashboard/static/dashboard/images/points.htm", "w") as f:
    #     f.write(svg_content)
...
def intermediate_points_generation(points1, points2, weight):
    ...
    """
    # points1 = smoothing(points1, iterations=10)
    # points2 = smoothing(points2, iterations=10)
    ...
    # x3, y3 = fft_reconstruct.intermediate_curve(x1, y1, x2, y2, weight)
    x3, y3 = point_reconstruct.intermediate_curve(x1, y1, x2, y2, weight)
    # x3, y3 = mra_reconstruct.intermediate_curve(y1, x1, y2, x2, weight)
    new_points = [(x3[i], y3[i]) for i in range(len(x3))]
    new_points = smoothing(new_points, iterations=10)
    return new_points
...
def smoothing(points, iterations=3):
